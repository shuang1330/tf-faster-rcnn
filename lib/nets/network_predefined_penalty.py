...
from pruning.activation_pruning import detect_diff_all

def activations_penalty_generator(x):
  # amplifier = 1.
  x[x<0] = 0
  penalty = tf.constant(x,tf.float32)
  return penalty

def activations_diff(hm_path):
  diff = detect_diff_all(hm_path,RANKING=False)
  return diff

def normed_sum(tensor):
  t_min, t_max = tf.reduce_min(tensor),tf.reduce_max(tensor)
...
class Network(object):
  ...
  def _reshape_layer(self, bottom, num_dim, name):
    ...
    with tf.variable_scope(name) as scope:
      ...
      reshaped = tf.reshape(to_caffe,
                            tf.concat(axis=0, values=[[self._batch_size],
                                [num_dim, -1], [input_shape[2]]]))
  ...
  def _crop_pool_layer(self, bottom, rois, name):
    with tf.variable_scope(name) as scope:
      batch_ids = tf.squeeze(tf.slice(rois, [0, 0],
                                        [-1, 1], name="batch_id"), [1])
  ...
  def _proposal_target_layer(self, rois, roi_scores, name):
    with tf.variable_scope(name) as scope:
      rois, roi_scores, labels, bbox_targets, \
        bbox_inside_weights, bbox_outside_weights = tf.py_func(
      ...
      bbox_inside_weights.set_shape([cfg.TRAIN.BATCH_SIZE,
                                     self._num_classes * 4])
  ...
  def _add_losses(self, sigma_rpn=3.0):
    with tf.variable_scope('loss_' + self._tag) as scope:
      ...
      loss_box = self._smooth_l1_loss(bbox_pred,
                                      bbox_targets,
                                      bbox_inside_weights,
      ...
      diff = activations_diff(hm_path)
      for act_i in range(len(self._act)):
	sums_activations = normed_sum(tf.reduce_sum(self._act[act_i],(0,1,2)))
	for j in range(len(self._act[act_i])):
	  penalty = activations_penalty_generator(diff['%dth_layer'%act_i][j])
	  sums_activations[j] = sums_activations[j]*penalty
        activation_loss += tf.reduce_mean(sums_activations)
...
#        activation_loss += tf.reduce_mean(weighted_sums_featuremaps)
  ...
  def create_architecture(self, sess, mode, num_classes, tag=None,
    ...
    #   for var in self._act: # only for vgg16
