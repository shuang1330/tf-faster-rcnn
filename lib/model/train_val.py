...
class SolverWrapper(object):
  ...
  def snapshot(self, sess, iter):
    ...
    if not os.path.exists(self.output_dir_penalty):
      os.makedirs(self.output_dir_penalty)

    # Store the model snapshot
    filename = cfg.TRAIN.SNAPSHOT_PREFIX + '_iter_{:d}'.format(iter) + '.ckpt'
    filename = os.path.join(self.output_dir_penalty, filename)
    self.saver.save(sess, filename)
    print('Wrote snapshot to: {:s}'.format(filename))

    # Also store some meta information, random state, etc.
    nfilename = cfg.TRAIN.SNAPSHOT_PREFIX + '_iter_{:d}'.format(iter) + '.pkl'
    nfilename = os.path.join(self.output_dir_penalty, nfilename)
    # current state of numpy random
    st0 = np.random.get_state()
    # current position in the database
    cur = self.data_layer._cur
...
#
      ...
      # Initialize all variables first
      print('initializing all variables first')
      variables = tf.global_variables()
      sess.run(tf.variables_initializer(variables, name='init'))
      print('initialized')
    ...
    # orginal implemantation, restore every variable found in the graph created
    # I cannot use it because of the tf.Variable intoduced for activation loss
    #   print('Restorining model snapshots from {:s}'.format(str(sfiles[-1])))
    ...
    #   print('Restored.')
      ...
      # Initialize all variables first
      sess.run(tf.variables_initializer(variables, name='init'))
    # original implementation, without activation losses
    # restore the random state of tf
    ...
    while iter < max_iters + 1:
      ...
      if now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
        ...
        rpn_loss_cls, rpn_loss_box, loss_cls,loss_box, act_loss, total_loss,summary = \
        ...
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, act_loss, total_loss = \
          self.net.train_step(sess, blobs, train_op)
      timer.toc()

      if act_loss < 0:
        raise ValueError('The activations should not be below zero.')

      # Display training information
      if iter % (cfg.TRAIN.DISPLAY) == 0:
        print('iter: %d / %d, total loss: %.6f\n >>> act_loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
              '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n >>> lr: %f' % \
              (iter, max_iters, total_loss, act_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, lr.eval()))
